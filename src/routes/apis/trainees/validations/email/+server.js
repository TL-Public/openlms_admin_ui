...
export async function GET({ request, fetch, cookies }) {
	...
	try {
		...
		const endPoint = `http://read-admin-api-dev.ap-south-1.elasticbeanstalk.com/apis/v1/trainees/emails/${email}`;
		...
		if (res.status === 200) {
			...
			return json({ status: 200, message: 'Username unavailable', traineeDetails:data },{ status: res.status }); // Return the existing user data
