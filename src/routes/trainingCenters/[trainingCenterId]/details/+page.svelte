<script>
	...
	let { tcCourseDetailsData, tcDetailsData, coursesData } = data;
	...
	// $: primaryDataError = tcDetailsData?.error ? tcDetailsData?.error : '';
	$: secondaryErrors = combineErrorMessages(
		tcDetailsData?.error,
		tcCourseDetailsData?.error,
		coursesData?.error
	);

	let formObjectCourse = {
		rsetiUuid: tcDetailsData?.uuid,
		tcName: tcDetailsData?.translations?.find((translation) => translation?.languageCode === 'en')
			?.name
	};

	if (!coursesData.error && !tcCourseDetailsData?.error) {
		let dataMap = {};
		...
		tcCourseDetailsData?.forEach((tc) => {
			...
			let tcObj = {
				...
				startDate: `${tc?.startMonth ?? '-'} / ${tc?.startYear ?? '-'}`, // Default to "-" if any data is missing
				startDateObj:
					tc?.startYear && tc?.startMonth
						? new Date(`${tc.startYear}-${String(tc.startMonth).padStart(2, '0')}-01`).getTime()
						: null,
	...
	function handleTableAction(event) {
		...
		if (actionName === 'view') {
			goto(
				`/trainingCenters/${data.rsetiUuid}/details/courseDetails?data=${encodeURIComponent(JSON.stringify(actionData))}`
			);
		}
		...
		if (actionName === 'edit') {
			...
			goto(
				`/trainingCenters/${data.rsetiUuid}/details/courseEdit?data=${encodeURIComponent(JSON.stringify(formObject))}`
			);
		}
	...
	function roleBasedAcessSetting() {
		...
		if (
			checkActionPermission(
				$userDetails?.role,
				moduleNames?.TRAINING_CENTER_COURSES,
				actionNames?.DETAILS
			)
		) {
			let tempObj = {
