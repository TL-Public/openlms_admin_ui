<script>
	...
	const videoHost = import.meta.env.VITE_KPOINT_URL || 'test-nabard.kpoint.com';
	// inform when mounted
	onMount(() => {
		mounted = true;
	});
...
<div id={divIdContainer} class="relative" style="width:100%; aspect-ratio: 16/9.5;">
	{#if !videoLoaded}
		...
		<div
			class="w-20 h-20 border-8 border-gray-30 border-t-accent z-5 absolute top-1/2 left-1/2 rounded-full -translate-x-1/2 -translate-y-1/2 round-loader origin-[0%_0%]"
		></div>
	{:else if videoFailed}
		<div
