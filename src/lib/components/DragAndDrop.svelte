...
>
	<div
		class="relative block w-full rounded-lg bg-offwhite border-2 border-dashed border-gray-300 p-12 py-20 text-center hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
		class:bg-white={file !== null}
		class:border-blue-500={dragover}
		on:click={() => document.getElementById(id).click()}
	>
		<input {id} type="file" accept=".xlsx" class="hidden" on:change={handleFileSelect} />
		...
		{:else}
			<div class="flex items-center justify-between bg-gray-100 p-2 rounded">
				...
				<button class="ml-2 text-red-600 hover:text-red-800" on:click|stopPropagation={removeFile}>
					<svg
		...
		{#if file !== null}
			<p class="mt-4 text-sm text-gray-600">Click or drag and drop to replace the file</p>
		{/if}
