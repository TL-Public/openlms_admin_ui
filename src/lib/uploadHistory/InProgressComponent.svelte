<script>
  ...
  $: visibleJobs = showAllJobs ? inProgressJobs : inProgressJobs.slice(0, INITIAL_JOBS_TO_SHOW);
  $: hasMoreJobs = inProgressJobs.length > INITIAL_JOBS_TO_SHOW;
  $: hiddenJobsCount = Math.max(0, inProgressJobs.length - INITIAL_JOBS_TO_SHOW);

  function calculateProgress(job) {
    if (!job.totalRows || job.totalRows === 0 || !job.processedRows) return 0;
  ...
  async function fetchInProgressJobs() {
    ...
    try {
      ...
      const jobsCompleted = Array.from(previousJobIds)?.some(id => !currentJobIds?.has(id));
...
<div class="mb-6">
  {#if loading && !error}
    <div class="bg-blue-50 border border-blue-200  mb-6 rounded-md p-3 flex items-center justify-center h-12">
      <div class="flex items-center gap-2">
        <GoogleMatrialIcon iconName="hourglass_empty" addClass="animate-spin text-primary text-sm" />
        <span class="text-xs text-gray-600">Loading in-progress uploads...</span>
      </div>
    </div>
  {:else if error}
    <div class="bg-red-50 border border-red-200 rounded-md p-3 h-12 mb-6 flex items-center justify-between w-full ">
      <div class="flex gap-2 items-center justify-between w-full">
        <div class="flex items-center gap-2">
        <GoogleMatrialIcon iconName="error" addClass="text-red-400 text-sm" />
        ...
        <Button 
          ...
          btnType="secondary"
        >
          <GoogleMatrialIcon iconName="refresh" addClass="text-xs" />
    ...
    <div class="mb-6">
      <div class="flex items-center justify-between mb-2">
        <h2 class="font-semibold text-primary flex items-center gap-2 text-sm">
          <GoogleMatrialIcon iconName="sync" addClass="animate-spin text-primary text-sm" />
      ...
      <div class="grid gap-2 mb-3">
        {#each visibleJobs as job, index (job.id)}
          <div class="bg-white border border-blue-100 rounded-md p-2.5 shadow-sm">
            ...
            <div class="mb-1.5">
              <div class="flex justify-between text-xs text-gray-500 mb-1">
                <span>Processed: {job.processedRows || 0} of {job.totalRows || '0'} rows</span>
                <span>{calculateProgress(job)}%</span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-1.5">
                <div 
                  class="bg-accent h-1.5 rounded-full transition-all duration-500 ease-in-out" 
            ...
            <div class="flex flex-wrap gap-3 text-xs text-gray-600">
              <div class="flex items-center gap-1">
                ...
                <span>Success: {job.successCount || 0}</span>
              </div>
              <div class="flex items-center gap-1">
