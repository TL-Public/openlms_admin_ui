<script>
	...
	function handleApplyFilter() {
		dispatch('handleFilters', { stateFilter: stateFilterValue });
	}
...
<div class="relative flex justify-end gap-8">
	<div class=" w-full flex sm:flex-row flex-col sm:gap-5 gap-2 justify-end flex-wrap">
		...
		<div class="flex justify-between gap-5">
			<button on:click={handleClearFilter} class="text-sm font-medium text-secondary">Clear</button>
			<button
				on:click={handleApplyFilter}
				class="text-sm rounded-[4px] font-medium text-white bg-primary px-6 py-2">Apply</button
			>
		</div>
	</div>
</div>
