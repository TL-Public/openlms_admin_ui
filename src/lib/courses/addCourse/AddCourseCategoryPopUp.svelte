	...
	async function handleSubmit() {
		...
		try {
			...
			if (formMode === 'PUT' || !extId) {
				...
				let englishEndpoint =
					method === 'POST' ? '/apis/courses/courseCategories' : `/apis/courses/courseCategories/${extId}`;
				...
				if (englishResponse?.status === 409) {
					...
					throw new Error(
						`Failed to ${method === 'POST' ? 'add' : 'update'} the course category. Please try again.`
					);
				} else {
					let englishResult = await englishResponse.json();
					extId = englishResult?.responseData?.extId; // Save extId for subsequent calls

					dispatch('handleAddCourseCategory', {
						englishResult: {
							extId,
							name: englishResult?.responseData?.name,
							languageCode: 'en'
						}
					});
				}
			...
			if (needsRetryHindi || formObject.titleHi) {
				...
				if (hindiResponse.status === 409) {
					...
					throw new Error(
						`Successfully ${method === 'POST' ? 'added' : 'updated'} the English category, but the Hindi category name already exists.`
					);
				} else if (!hindiResponse.ok) {
					needsRetryHindi = true; // Mark Hindi submission as needing retry
					throw new Error(
						`Successfully ${method === 'POST' ? 'added' : 'updated'} the English category, but failed to ${method === 'POST' ? 'add' : 'update'} the Hindi category.`
					);
				} else {
					...
					dispatch('handleAddCourseCategory', {
						hindiResult: {
							extId,
							name: hindiResult?.responseData?.name,
							languageCode: 'hi'
						}
					});
	...
	>
		<div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
			...
			>
				...
				{/if}
				<div class="mb-2">
					<h2 class=" heading-L">{formMode === 'POST' ? 'Add' : 'Edit'} Course Category</h2>
					<div class="mt-4">
						<InputField
