<script>
	...
	export let moduleFilterValue;

	// gets option lists from +page.svelte
	export let moduleFilterOptionList = [];
	const dispatch = createEventDispatcher();

	// dipatches event when filters are applied or cleared
	function handleApplyFilter() {
		dispatch('handleFilters', { moduleFilter: moduleFilterValue });
	}

	function handleClearFilter() {
		moduleFilterValue = String_Constants.ALL_MODULES;
		dispatch('handleFilters', { moduleFilter: moduleFilterValue });
	}
...
<div class="relative flex justify-end">
	<div class=" w-full flex sm:flex-row flex-col sm:gap-5 gap-2 justify-end flex-wrap">
		<Filter
			bind:itemSelected={moduleFilterValue}
			optionList={moduleFilterOptionList}
			optionListConfigObject={moduleFilterConfig}
