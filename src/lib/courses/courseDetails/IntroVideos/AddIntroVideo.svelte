<!-- As per design, this pop up is only supporting add, can be scaled to add edit as well -->
...
<script>
	...
	import { getErrorMessage, handleRedirection, extractYouTubeVideoId } from '$lib/utils/helper.js';
	...
	export let availableLanguagesForAdd = []
	export let selectedLanguage=selectedVideo?.languageCode || ''
	...
	function getCodeByName(name) {
		for (let item of availableLanguagesForAdd) {
	...
	async function handleSubmit() {
		...
		try {
			...
			const duplicate = introVideos?.some((video) => extractYouTubeVideoId(video?.url) === extractYouTubeVideoId(formObject?.url) || extractYouTubeVideoId(video?.videoUrl) === extractYouTubeVideoId(formObject?.url));
			...
			response = await fetch(
				`/apis/courses/details/${courseUuid}/introVideos`,
				{
					method: 'POST',
			...
			>
				...
				<div class="mb-2">
					<div class="flex gap-1 items-center">
						...
						<h2 id="modal-title" class=" font-semibold capitalize">{mode} Introductory Video</h2>
					</div>
					<div class="mt-4">
						<InputField
							label={'Video URL'}
					...
					<div class="mt-2 w-full">
						...
						<DropDown
							...
							options={availableLanguagesForAdd}
