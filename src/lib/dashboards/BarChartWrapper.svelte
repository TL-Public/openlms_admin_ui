<script>
	...
	$: hasAllZeroData = chartData?.datasets?.every(dataset => 
...
<div class="rounded-lg border border-gray-200 bg-white shadow-sm p-4 md:pt-6 md:px-4">
	<!-- Card Header -->
	<div class="mb-4">
		<CardHeader {title} {subtitle} {showFilters} {filters} on:filterChange {selectedGlobalFilters}/>
	</div>
	...
	<div class="w-full">
		{#if isLoading}
			<div
				class="rounded-lg border border-gray-200 bg-white shadow-sm p-4 md:pt-6 md:px-4 min-h-80 flex justify-center items-center w-full"
			>
				<Spinner size={60} />
			</div>
		{:else if error}
